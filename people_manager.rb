...
class PeopleManager
    ...
    def load_people
        if File.exist?('people.json')
            if File.size('people.json') == 0
                @people_list = []
            else
                catched_data = JSON.parse(File.read('people.json'))
                @people_list = catched_data.map do |person|
                    if person['class'] == 'Student' && person['parent_permission'] == true
                       Student.new(person['age'], person['name'], parent_permission: true, id: person['id'])
                    elsif person['class'] == 'Student' && person['parent_permission'] == false
                        Student.new(person['age'], person['name'], parent_permission: false, id: person['id'])
